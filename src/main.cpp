...
uint256 hashGenesisBlock("0x000005a1989cc2b97a04c97da36a8e601247c710d48db22bbcc332b2ec7bdb11");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SunWenCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SunWenCoin Signed Message:\n";
...
{
    // SunWenCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight == 1)
		return 99500000000 * COIN;

    nSubsidy >>= (nHeight / 83333334);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60;
static const int64 nTargetSpacing = 88;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 88;
...
{
    // SunWenCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "swc genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1485289455;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3236732;
        ...
        assert(block.hashMerkleRoot == uint256("0xaaf7890f89c150d79a7c5ea202bccf1bf3b2aa7e207e04a63f2c71ade49ceb76"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // SunWenCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SunWenCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SunWenCoinMiner : ProcessBlock, block not accepted");
    }
